...
def get_safaricom_stock_data(request):
    try:
        ...
        if data.empty:
            return JsonResponse({'error': 'No stock data found. Check the ticker symbol and date range.'}, status=404)
        ...
        stock_data = {
            ...
            'avg_volume': sum(data['Volume']) / len(data['Volume']) if 'Volume' in data and len(data['Volume']) > 0 else 0,
        }
